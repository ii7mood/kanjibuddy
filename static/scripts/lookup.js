...
export async function toggle_save_mode(button) {
  ...
  if (button.classList.contains("save-mode")) {
    // already in save-mode meaning we gotta update and return to normal view
    const editable_cells = row.querySelectorAll(".edit-box");
    const [kanji, meaning, onyomi, kunyomi] = Array.from(editable_cells).map(
      (i) => i.value
    );
  ...
  for (let i = 2; i < cells.length - 1; i++ + 1) {
    // ignore the first two cells (checkbox and id) and the last one (modify button)
