async function list_cards(n) {
  ...
  if (!response.ok) {
    window.display_message(
      "Failed to get database records; refer to console logs.",
      "negative"
    );
...
document
  .querySelector("#marker-correct")
  .addEventListener("click", () => mark_card(true));
document
  .querySelector("#marker-incorrect")
  .addEventListener("click", () => mark_card(false));
